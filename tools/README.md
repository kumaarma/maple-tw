...
`--mode single` 測的是單件試算有沒有真的把表單的值算進判定。這件事光看截圖
分不出來 —— 表單沒接上的話畫面照樣渲染，只是判定永遠停在「數值相同」。所以
會填一個已知的差值（星力改 25）確認判定會變、逐項差異列得出星力，再填一條
看得懂的潛能與一條看不懂的，確認前者進判定、後者只標 ✗ 而不進判定。另外驗
判定用的 5 格與「其他數值」那 8 格有分開、預設是收合的，以及空表單的狀態：
欄位全空、不給判定、只顯示一欄（不該冒出全 0 的「試算裝備」）、有提示。最後
帶入一件**不同欄位**的裝備，確認跨欄位比對真的成立 —— 那正是逐格比對做不到的事。
...
| `harness/driver.js` | 點分頁、切配對模式、填表單、收集結果 |
