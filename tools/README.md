...
`--mode soul` 測靈魂武器那一段有沒有顯示出來（裝備併進總覽後，這個模式改點
「總覽」而不是已經不存在的「裝備」分頁）。順便驗一般裝備的詳情彈窗：點得開、
有數值拆解、而且分得出基礎／追加／卷軸／星力 —— 這個彈窗之前只有靈魂武器那
一塊被測到，等於彈窗整個不開也照樣過關。另外驗「目前穿戴」有沒有機器人那一格
且有圖示：機器人是從另一個端點補進去的，補漏了那格只會安靜地變空，畫面照樣
好好的，沒有任何檢查會失敗。改版後 API 換成 `soul_weapon_*`
...
**假資料的基準日要跟 `latestDataDate()` 一模一樣。** 官方資料凌晨才更新，
所以台灣時間 2 點前算的是「前天」。`--mode exp` 的 `seedExp()` 一開始只減
1 天，在 00:00～01:59 這個窗口會跟畫面差一天，最舊那筆掉出查詢範圍，七天
變六天。寫進來之後每次都在白天跑，一直沒被抓到 —— 跟時間有關的假資料要照
抄產品端的規則，不能自己另算一套。
